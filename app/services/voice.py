...
# WebUI 的“自定义音频”上传控件接受这一组扩展名，并按原扩展名存进任务目录；
# 这里必须覆盖同一组格式，否则上传 wav/m4a 之类的音频会被判定为 0 时长，
# 任务直接失败。扩展名比较统一转小写，兼容 .MP3 这种大写后缀。
...
def get_audio_duration(target: Union[str, SubMaker]) -> float:
    ...
    elif isinstance(target, str) and target.lower().endswith(AUDIO_FILE_EXTENSIONS):
